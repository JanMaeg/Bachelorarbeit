...
class CorefModel(nn.Module):
    def __init__(self, config, device, num_genres=None):
        ...
        self.emb_span_width = self.make_embedding(config["span_width_embedding_size"]) if config[
            'use_features'] else None
        self.emb_span_width_prior = self.make_embedding(config["span_width_embedding_size"]) if config[
            'use_width_prior'] else None
        self.emb_antecedent_distance_prior = self.make_embedding(config['num_antecedent_distance_buckets']) if config[
        ...
        self.emb_cluster_size = self.make_embedding(10) if config['fine_grained'] and config[
            'higher_order'] == 'cluster_merging' else None
        ...
        self.span_attn_ffnn = self.make_ffnn(self.span_emb_size, 0, output_size=1) if config['fine_grained'] and config[
        ...
        self.cluster_score_ffnn = self.make_ffnn(3 * self.span_emb_size + config['feature_emb_size'],
            ...
            config['fine_grained'] and config['higher_order'] == 'cluster_merging' else None
    ...
    def load_state_dict(self, state_dict: 'OrderedDict[str, Tensor]',
        ...
        if 'emb_antecedent_distance_prior.weight' in state_dict and state_dict[
            'emb_antecedent_distance_prior.weight'].shape != new_shape:
        ...
        if emb_segment_distance_weight in state_dict and state_dict[emb_segment_distance_weight].shape != new_shape:
            logger.warn(
                'Saved embedding for segment distance is of different size, some values are initialized randomly.')
    ...
    def get_candidate_spans(self, num_words, sentence_map, gold_info, device='cpu'):
        ...
        if gold_info is not None:
            ...
            candidate_labels = torch.matmul(torch.unsqueeze(gold_info['gold_mention_cluster_map'], 0).to(torch.float),
    ...
    def build_candidate_embeddings(self, mention_doc, candidate_spans, device='cpu'):
        ...
        candidate_tokens = torch.unsqueeze(torch.arange(0, num_words, device=device), 0).repeat(num_candidates,
                                                                                                1)  # [num_candidates, num_words]
    ...
    def get_predictions_and_loss(self, input_ids, input_mask, speaker_ids, sentence_len, genre, sentence_map,
        ...
        if conf['fine_grained']:
            ...
            if conf['use_features']:  # Antecedent distance
                top_antecedent_distance = util.bucket_distance(top_antecedent_offsets, num_buckets=self.config[
                    'num_antecedent_distance_buckets'])
        ...
        if self.debug:
            if self.update_steps % 20 == 0:
                ...
                logger.info('spans/gold: %d/%d; ratio: %.2f' % (
                    num_top_spans, (top_span_cluster_ids > 0).cpu().numpy().sum(),
                    (top_span_cluster_ids > 0).cpu().numpy().sum() / num_top_spans))
                if conf['mention_loss_coef']:
                    logger.info('mention loss: %.4f' % loss_mention.item())
                if conf['loss_type'] == 'marginalized':
                    logger.info('norm/gold: %.4f/%.4f' % (
                        torch.sum(log_norm).item(), torch.sum(log_marginalized_antecedent_scores).item()))
                else:
...
class MentionModel(CorefModel):
    ...
    def forward(self, input_ids, input_mask, speaker_ids, sentence_len, genre, sentence_map,
        ...
        span_start_emb, span_end_emb = mention_doc[candidates['candidate_starts']], mention_doc[
...
class IncrementalCorefModel(CorefModel):
    ...
    def update_evaluator(self, span_starts, span_ends, predicted_clusters, mention_to_cluster_id, gold_clusters,
                         evaluator):
        mention_to_predicted = {m: tuple(predicted_clusters[cluster_idx]) for m, cluster_idx in
    ...
    def get_predictions_incremental(self, input_ids, input_mask, speaker_ids, sentence_len, genre, sentence_map,
        ...
        for i, length in enumerate(segments_per_split):
            ...
            if gold_ends is not None:
                ...
                gold_mask = (gold_starts < end_offset) & (gold_starts > start_offset) | (
                        (gold_ends > start_offset) & (gold_ends < end_offset))
            ...
            windowed_predictions_starts = predictions_starts[predictions_split_map == i]
            windowed_predictions_ends = predictions_ends[predictions_split_map == i]
            windowed_predictions_cluster_map = predictions_cluster_map[predictions_split_map == i]

            # sort them by index
            windowed_predictions_starts, indices = torch.sort(windowed_predictions_starts)
            windowed_predictions_ends = windowed_predictions_ends[indices]
            windowed_predictions_cluster_map = windowed_predictions_cluster_map[indices]
            ...
            res = self.get_predictions_incremental_internal(
                ...
                predictions_starts=windowed_predictions_starts,
                predictions_ends=windowed_predictions_ends,
    ...
    def get_predictions_spans(self, predictions_starts, predictions_ends):
        return {
            ...
            'candidate_ends': predictions_ends,
            'num_candidates': predictions_starts.size()[0],
            'candidate_labels': None,
        }

    def get_predictions_incremental_internal(self, input_ids, input_mask, speaker_ids, sentence_len, genre,
                                             sentence_map,
                                             is_training, gold_starts=None, gold_ends=None,
                                             gold_mention_cluster_map=None,
                                             entities=None, do_loss=None, offset=0,
                                             loss_strategy=GoldLabelStrategy.MOST_RECENT,
                                             teacher_forcing=False,
                                             predictions_starts=None,
                                             predictions_ends=None,
                                             predictions_cluster_map=None):
        device = self.device
        conf = self.config
        ...
        if do_loss:
            ...
            labels_for_starts = {(s.item(), e.item()): v.item() for s, e, v in
                                 zip(gold_starts, gold_ends, gold_mention_cluster_map)}
        else:
            gold_info = None
            labels_for_starts = {}
...
#                                       dim=1)  # [num candidates, new emb size]
        ...
        if entities is None:
            ...
            # For the first split we simply add all entities like they are because we already have the belonging clusters
            for emb, span_start, span_end, cluster_idx in zip(predictions_span_emb, predictions_starts, predictions_ends, predictions_cluster_map):
                if conf['evict']:
                    entities.evict(evict_to=cpu_entities)

                if len(entities.count) <= cluster_idx:
                    entities.add_entity(emb, cluster_idx, span_start, span_end, offset=offset)
                else:
                    update_gate = torch.sigmoid(
                        self.entity_representation_gate(torch.cat(
                            [
                                emb,
                                entities.emb[cluster_idx],
                            ],
                        )))
