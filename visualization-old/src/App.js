  ...
  for (let i = 0; i < subtokens.length; i++) {
    ...
    if (i === subtokens.length - 1 || subtokenMap[i + 1] > subtokenMap[i]) {
      ...
      if (Array.isArray(sentences[sentenceIndex])) {
        sentences[sentenceIndex].push({
          word: token,
          clusters: getClusterOfToken(i, data.clusters),
        });
      } else {
        sentences[sentenceIndex] = [
          { word: token, clusters: getClusterOfToken(i, data.clusters) },
        ];
      }
  ...
  return (
    <p>
      {words.map((word, index) => {
        const clusters = showMergedClusters
          ? word.merged_clusters
          : word.clusters;
        ...
        if (PUNCTUATIONS.includes(word.word) || index === words.length - 1) {
          return (
            <span key={index} style={getStyle(clusters)}>
              {word.word} {clusterIdText}
            </span>
          );
        }
        ...
        return [
          <span key={index}> </span>,
          <span key={`2-${index}`} style={getStyle(clusters)}>
            {word.word} {clusterIdText}
          </span>,
        ];
      })}
      ...
      <div className={`result ${showBoundaries && "result--boundaries"}`}>
        {documents.map((document, index) => (
          <div className="document" key={index}>
            {document.map((sentence, sentenceIndex) => (
              <Sentence
                words={sentence}
                key={sentenceIndex}
                showMergedClusters={showMergedClusters}
              />
            ))}
          </div>
        ))}
  ...
  return (
    <div className="result document">
      {data.map((sentence, index) => (
        <Sentence key={index} words={sentence} />
      ))}
    </div>
  );
}
    ...
    <div>
      <div className="controls">
        ...
        <label>
          ...
          />
          Don't use the following tokens for merge: ["er", "sie", "sich",
          "sein", "seinem", "ihre", "ihr", "Sie", "Ich", "ich", "ihm", "ihn",
          "seine", "ihres", "seinen", "ihrer", "ihrem", "seiner", "ihren"]
        </label>
      </div>
      <div className="App">
        {/*
